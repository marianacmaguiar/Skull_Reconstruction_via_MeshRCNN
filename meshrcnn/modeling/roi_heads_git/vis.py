...
def visualize_minibatch(images, data, output_dir, vis_fg=False):
    ...
    #print("Check if there are target_meshes in data: ",len(data["target_meshes"]))
    #print("Check if there are target_masks in data: ",len(data["target_masks"]))
    #print("Check if there are target_voxel in data: ",len(data["target_voxels"]))

    
    if "target_meshes" in data:
        target_sampled_verts = sample_points_from_meshes(
            data["target_meshes"], num_samples=20000, return_normals=False
        )
    ...
    for i in range(num):
        ...
        for j in range(boxes.shape[0]):
            ...
            if vis_fg or "target_meshes" in data:
                resolution = 28
            ...
            if vis_fg or "init_meshes" in data:
                resolution = 28
