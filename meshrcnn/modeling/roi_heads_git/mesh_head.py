...
from mesh import MeshInstances, batch_crop_meshes_within_box
...
def mesh_rcnn_loss(
    ...
    "CHANGED GT_NUM_SAMPLES AND PRED_NUM_SAMPLES FROM 5000 TO 4000"
...
class MeshRefinementStage(nn.Module):
    ...
    def forward(self, x, mesh, vert_feats=None):
        ...
        #final_obj = os.path.join('output/training_meshes/', 'MeshRefinementStage_foward'+str(counter)+ '.obj')
...
class MeshRCNNGraphConvHead(nn.Module):
    ...
    def forward(self, x, mesh):
        ...
        for stage in self.stages:
            ...
            # Store the predicted mesh using save_obj
            #final_obj = os.path.join('output/training_meshes/', 'MeshRCNNGraphConvHead_foward'+str(counter)+ '.obj')
            #save_obj(final_obj, final_verts, final_faces) 
