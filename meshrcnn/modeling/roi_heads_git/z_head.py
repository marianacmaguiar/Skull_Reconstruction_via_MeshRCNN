...
def z_rcnn_loss(z_pred, instances, src_boxes, loss_weight=1.0, smooth_l1_beta=0.0):
    ...
    cls_agnostic_z = z_pred.size(1) == 1  #True
    ...
    for instances_per_image in instances:
        #print("len(instances_per_image) : ", len(instances_per_image))                            # =1
        if len(instances_per_image) == 0:
            continue
        #print("cls_agnostic_z: ", cls_agnostic_z)                                                 # = True
        if not cls_agnostic_z:
            gt_classes_per_image = instances_per_image.gt_classes.to(dtype=torch.int64)
            gt_classes.append(gt_classes_per_image)  

        
        #print("instances_per_image.gt_dz: ", instances_per_image.gt_dz)            # tensor([-773.3583], device='cuda:0')
    ...
    # since cls_agnostic_z = True, z_pred = z_pred[:, 0]
    if cls_agnostic_z:
        z_pred = z_pred[:, 0]
    else: # DOESN'T RUN
    ...
    loss_z_reg = loss_weight * loss_z_reg / len(gt_classes)
