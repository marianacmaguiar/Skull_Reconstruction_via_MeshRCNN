...
class Pix3DEvaluator(DatasetEvaluator):
    ...
    def evaluate(self):
        ...
        """ self._predictions returns results """    
            
        self._results = OrderedDict()
       
        if "instances" in self._predictions[0]:
            
            "Until here it runs, fails in self._eval_predictions, which doesn't run at all!"
            " print(self._predictions[0] also works! )"
    ...
    def _eval_predictions(self):
        ...
        if "segm" in self._tasks or "mesh" in self._tasks:
            ...
            print("results from _eval_preditcitons: ",results)
...
def transform_meshes_to_camera_coord_system(meshes, boxes, zranges, Ks, imsize):
    ...
    for i in range(len(meshes)):
        ...
        save_obj(final_obj, new_verts, new_faces)
